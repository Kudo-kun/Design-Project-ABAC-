...
struct ABAC_POLICY_MINER
{
    ...
    void rule_generator()
    {
        for(int i = 0; i < us; i++)
            for(int j = 0; j < obj; j++)
            {
                ...
                bool present = (find(eff_rules.begin(), eff_rules.end(), r) != eff_rules.end());
    ...
    void merge_rules()
    {
        ...
        {
            bool remove_ith = 0;
            for(int j = i+1; j < (int)eff_rules.size(); j++)
                if(can_be_merged(eff_rules[j], eff_rules[i]))
                    merger(eff_rules[j], eff_rules[i]), remove_ith = 1;
    ...
    {
    	int max_rule_weight = (int)eff_rules[0].size();
    	int bound = (percent * max_rule_weight);
    	cout << "bound = " << bound << '\n';
    	for(int i = 0; i < (int)eff_rules.size(); i++)
    		if(accumulate(eff_rules[i].begin(), eff_rules[i].end(), 0) > bound)
    			eff_rules.erase(eff_rules.begin() + i), i--;
